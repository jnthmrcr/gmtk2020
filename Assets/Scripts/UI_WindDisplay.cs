...
public class UI_WindDisplay : MonoBehaviour
{
	...
	{
		nodeSmall[0].position = myTrans.position;
		int index = 1;

		for (int i = 1; i < Mathf.Abs(x) + 1; i++)
		{
			nodeSmall[index].gameObject.SetActive(true);
			nodeSmall[index].anchoredPosition = myTrans.anchoredPosition + new Vector2(Mathf.Sign(x) * i * 20f, 0);
			index++;
		}
		...
		{
			nodeSmall[index].gameObject.SetActive(true);
			nodeSmall[index].anchoredPosition = myTrans.anchoredPosition + new Vector2(x * 20f, Mathf.Sign(y) * i * 20f);
			index++;
		}
		...
		nodeLarge.anchoredPosition = myTrans.anchoredPosition + new Vector2(x * 20f, y * 20f);

		connectorLine.transform.localScale = new Vector3(Vector2.Distance(Vector2.zero, new Vector2(x, y)), 1, 1);
		connectorLine.anchoredPosition = myTrans.anchoredPosition + new Vector2(x * 10f, y * 10f);
		connectorLine.localRotation = Quaternion.Euler(0, 0, Mathf.Rad2Deg * Mathf.Acos((float) x / (float) y));
	}
}